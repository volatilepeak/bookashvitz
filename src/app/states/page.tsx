...
export default async function StatesPage() {
  ...
  // Add states from TOP_CITIES that aren't in DB yet
  const dbStateSlugs = new Set(states.map(s => s.state_slug))
  const extraStates = TOP_CITIES
    .filter(c => !dbStateSlugs.has(c.stateSlug))
    .reduce((acc, c) => {
      if (!acc.find(s => s.stateSlug === c.stateSlug)) {
        acc.push(c)
      }
      return acc
    }, [] as typeof TOP_CITIES)
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          {extraStates.map(state => (
            ...
            >
              <h2 className="font-display font-semibold text-lg">{state.state}</h2>
              <p className="text-sm text-stone-400 mt-1">Coming soon</p>
            </Link>
          ))}
        </div>

        {states.length === 0 && extraStates.length === 0 && (
